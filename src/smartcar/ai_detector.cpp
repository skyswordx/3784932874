 ...
 */
void AIDetector::init()
{
    std::string pathModel = "../res/model/yolov3_mobilenet_v1";
    // 模型初始化
...
std::vector<PredictResult> AIDetector::inference(cv::Mat img)
{
...
void AIDetector::buildNms(const std::string &model_dir)
{
...
void AIDetector::transposeAndCopyToTensor(const cv::Mat &src, NDTensor &dst)
{
    cv::Mat channels[3];
    split(src, channels);
...
std::shared_ptr<std::unordered_map<std::string, NDTensor>> AIDetector::preprocess(
    cv::Mat frame,
    const std::vector<int64_t> &input_size)
{
    ...
    // 增加亮度
    // x += cv::Scalar(0.3, 0.3, 0.3); // 增加亮度值，可根据需要调整
...
void AIDetector::run(const std::unordered_map<std::string, NDTensor> &feeds)
{
...
NDTensor AIDetector::get_output(int index)
{
    return this->predictor_nms_->get_output(index);
}

std::vector<PredictResult> AIDetector::render()
{
    ...
    std::vector<PredictResult> result_list;
    PredictResult result;
    for (int i = 0; i < prod; i += 6)
    {
        result.type = data[i];
        result.score = data[i + 1];
        // std::cout << "LABEL: " << result.type << " SCORE: " << result.score << std::endl;
        if (result.score < score && result.type != 11) // 阈值
        {
            continue;
        }
...
* @brief 绘制AI模型识别结果
*/
void AIDetector::drawBox(cv::Mat &img, std::vector<PredictResult>& results)
{
 ...
 * @return cv::Scalar
 */
cv::Scalar AIDetector::getCvcolor(int index)
{
    switch (index)
    {
    case 0:
